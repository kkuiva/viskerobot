...
#define g 9.8067
//eeldatav viskamise nurk (rad, 45 kraadi)
#define release_angle 0.7854  
//kaugus kuubiku ja roboti 4. liigendi vahel, kasutatakse joonkiiruse nurkkiiruse teisendamiseks
...
public:
  ...
  : Node("xarm_api_demo")
  {
    // Namespace prefix used by xarm_api, default is usually "/xarm"
    ...
    //roboti asendid: koduasend, asend kuubiku kohal enne haaramist, viskamise lõppasend
    ...
    joint_pos_3_ = { 0, 0, 0, PI/2, 0, 0, 0 };

    //kuubiku haaramise positsioon
  ...
  bool run_demo()
  {
    ...
    while(true){
      float throw_speed = calculate_speed();
      int release_delay = calculate_release_speed(throw_speed);
        
      RCLCPP_INFO(get_logger(), "Arvutatud kiirus: %.1f", throw_speed);
      RCLCPP_INFO(get_logger(), "Arvutatud paus: %d", release_delay);
      ...
      if (!move_joint(joint_pos_1_, 1, 15.0, true)) {
        return false;
      }
        
      //viske lõppasend
      if (!move_joint(joint_pos_3_, throw_speed, 100.0, false)) {
        return false;
      }
      ...
      int uus_vise;
      std::cout << "Uus vise? (1/0)";
      std::cin >> uus_vise;

      if (uus_vise != 1){
        RCLCPP_INFO(get_logger(), "Koduasendisse liikumine");
          //koduasend
      ...
      RCLCPP_INFO(get_logger(), "Uus vise");
    }
  }
...
private:
  ...
  //funktsioon vajaliku viskamiskiiruse arvutamiseks
  float calculate_speed(){
    float dist = 0.0;
    std::cout << "Topsi kaugus meetrites (max 1.09):";
    std::cin >> dist; //kasutaja sisend soovitud kauguse kohta
    
    dist = dist - 0.55; //kauguse roboti baasist viskepunkti teisendamine
    float y = -0.15; //sihtmärgi kõrgus, kui viskamise koht on y = 0 (m)

    //viskamiseks vajaliku joonkiiruse arvutamine
    float linear_speed = sqrt((dist * dist * g) / ((dist * sin(2*release_angle)) - (2 * y * cos(release_angle) * cos(release_angle))));
    //joonkiiruse nurkkiiruseks teisendamine
    float throw_speed = linear_speed / throw_radius;

    return(throw_speed);
  }

  //funktsioon haaratsi ajastamiseks vajaliku ooteaja arvutamiseks
  int calculate_release_speed(float throw_speed){
    int action_delay; //kasutaja sisestatud lisaaeg
    std::cout << "Vajalik lisaaeg:";
    std::cin >> action_delay;
    
    //viskamise alguspunktist viskepunkti jõudmiseks kuluv aeg
    int toss_time = (release_angle/throw_speed) * 1000; 

    //liikumisele kuluvale ajale lisatakse kasutaja sisestatud lisaaeg
    int release_delay = toss_time + action_delay;
  ...
  { //kontrollib saadud asendite vastavust roboti liigendite arvuga
    ...
    RCLCPP_INFO(
      get_logger(),
      "Poosi liikumine: kiirus=%.3f kiirendus=%.3f ootamine=%d", speed, acc, wait);
  ...
  bool move_cartesian(const std::vector<float> & pose, float speed, float acc, bool wait)
  {
    ...
    RCLCPP_INFO(
      get_logger(),
      "Positsiooni liikumine: kiirus=%.1f kiirendus=%.1f ootamine=%d", speed, acc, wait);
    //tellimus edastatakse call_simple funktsiooniga
